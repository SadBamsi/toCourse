...
document.addEventListener('DOMContentLoaded', function() {
  ...
  mobileBtn.addEventListener('click', () => {
    ...
    let picturesZone = document.querySelector('.carousel_picture-zone');
    let gap = parseInt(window.getComputedStyle(picturesZone).getPropertyValue('column-gap'), 10);
    ...
    let petAvatar = document.querySelectorAll('.card_photo');
    let petName = document.querySelectorAll('.card_name');
    let petCard = document.querySelectorAll('.card');
    ...
    function drowSlider(breakpoint) {
      ...
      for (let i = 0; i < petAvatar.length; i++) {
        petAvatar[i].src = arr[i].img;
        petName[i].textContent = arr[i].name;
        petCard[i].dataset.id = arr[i].index;
      }
    }

    drowSlider(breakpoint)
